...
def patient_nifti_to_png(nifti_path, patient_name, result_path):
    ...
    for idx, img_slice in enumerate(nii_array):
        ...
        # Normalize the slice to 8-bit grayscale
        ...
        img_slice = Image.fromarray(img_slice).convert('L')
        # Plot the image
        if idx == 110:
...
def padding(image, new_size):
    ...
    padded_img = np.array(padded_img).astype(np.uint8)
    return padded_img
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        result_dataset_path = f'/Users/alexboving/Desktop/Thèse/synthrad-pix2pix/{task}_8bit_gray/{structure}_{modality}/'
